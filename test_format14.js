...
const solve = (r, c, idx) => {
  ...
  if (matrix[r][c] !== null) {
    ...
    // if it matches, OR if the list has a null here, we can skip it.
    if (idx < lists[r].length) {
      if (lists[r][idx] === matrix[r][c]) {
        return solve(r, c + 1, idx + 1);
      }
      if (lists[r][idx] === null) {
        // Try skipping the null
        if (solve(r, c, idx + 1)) return true;
      }
    }
    return false;
  }
  ...
  if (idx < lists[r].length) {
    ...
    if (val !== null) {
      ...
      // If the list has a null, we can skip it and try assigning the next value
      if (solve(r, c, idx + 1)) return true;
    }
  }
  return false;
