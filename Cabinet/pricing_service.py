"""
Единый расчёт стоимости подписки: тариф + реферальная скидка + промокод.
...
def calculate_subscription_price(
    ...
    *,
    ...
    applied_discount_source: "referral" | "promo" | "none"
    """
    ...
    # Выбор наиболее выгодной скидки (referral и promo не суммируются).
    applied_source = "none"
    ...
    if promo_breakdown is not None:
        # FREE_MONTHS с final=0 нельзя оплатить через эквайринг — не выбираем как победителя
        # для create_payment (обработаем отдельно). Для сравнения выгодности используем discount.
        candidates.append(
            {
                "source": "promo",
    ...
    if candidates:
        # Максимальная скидка; при равенстве предпочитаем referral (явный приоритет программы).
        best = max(
            candidates,
            key=lambda c: (c["discount"], 1 if c["source"] == "referral" else 0),
        )
        # Если промо даёт final=0 (free months) — это «бесплатно», но create_payment отвергнет.
        # Всё равно отражаем в preview.
    ...
    message = None
    if applied_source == "referral" and promo_breakdown is not None:
        message = (
    ...
    return {
        ...
        "promo_code": applied_promo_code if applied_source == "promo" else (
