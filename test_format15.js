...
const solve = (r, c, idx) => {
  ...
  if (c === r) {
    // Diagonal must be null in the matrix, but the list might have a null for it
    if (idx < lists[r].length && lists[r][idx] === null) {
  ...
  if (matrix[r][c] !== null) {
    // If the matrix already has a value here, we MUST consume it from the list
  ...
  // matrix[r][c] is null. We can either leave it null...
  // If we leave it null, we must consume a null from the list if there is one
  if (idx < lists[r].length && lists[r][idx] === null) {
    if (solve(r, c + 1, idx + 1)) return true;
  } else {
    // Or if the list doesn't have a null here, maybe it's just not in the sparse list?
    // Wait, if we are normalizing a sparse list with nulls included, we MUST consume exactly what's there.
  }

  // ...or assign it a value from the list
