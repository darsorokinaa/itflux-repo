...
from Generator.latex_utils import process_latex, _repair_unclosed_dollar_math
...
class DollarFormulaLatexTests(SimpleTestCase):
    ...
    def test_process_latex_unclosed_dollar_skanavi(self):
        """В БД у Сканави часто только открывающий $ — формула не должна рваться."""
        raw = (
            r"$\left(\sqrt{\dfrac{(1-n)\sqrt[3]{1+n}}{n}}"
            r"\cdot\sqrt[3]{\dfrac{3n^2}{4-8n+4n^2}}\right)^{-1}"
            r":\sqrt[3]{\left(\dfrac{3n\sqrt n}{2\sqrt{1-n^2}}\right)^{-1}}"
        ...
        self.assertNotIn("&#92;($", out)
        self.assertEqual(out.count("<span"), 1)
        self.assertNotIn(r"\right&#92;)", out)

    def test_process_latex_unclosed_dollar_sqrt_b(self):
        raw = (
            r"$\left(\dfrac{a+a^{3/4}b^{1/2}+a^{1/4}b^{3/2}+b^2}"
            r"{a^{1/2}+2a^{1/4}b^{1/2}+b}(\sqrt[4]{a}+\sqrt b)"
            r"+\dfrac{3\sqrt b(a^{1/2}-b)}{a^{-1/4}(a^{1/4}-\sqrt b)}"
    ...
    def test_repair_unclosed_skips_excel_and_symbol_mentions(self):
        excel = '<p><span>=$</span><span>D</span><span>1+B$1</span></p>'
        self.assertEqual(_repair_unclosed_dollar_math(excel), excel)
        symbol = 'Символы «&», «#», «$», «*».'
        self.assertEqual(_repair_unclosed_dollar_math(symbol), symbol)
        latex = r"$\dfrac{8-m}{\sqrt[3]{m}+2}"
        self.assertEqual(_repair_unclosed_dollar_math(latex), latex + "$")
