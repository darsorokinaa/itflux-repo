# Связка ЛК (`02_lk_generator`) и генератора (itflux)

## Два репозитория

| Проект | Роль |
|--------|------|
| Соседняя папка `../02_lk_generator` | Кабинет: `POST /api/lesson/token/` выдаёт JWT и URL `ITFLUX_URL/lesson/join/?token=…` |
| Этот репозиторий | itflux: проверка JWT, страница урока, API задач |
...
- **`LESSON_SECRET`** — как в `02_lk_generator/Cabinet/.env.example` и в `systemctl cat itflux`.
- **`ITFLUX_URL`** (или аналог в ЛК) — точный URL сайта генератора (например `https://tes.itflux.ru`).
...
1. **Один порт у Daphne и у nginx**  
   ...
   В активном `nginx` для itflux все `proxy_pass` на этот порт (в репозитории — `upstream itflux_asgi` в `deploy/nginx.conf`; при необходимости замените `8002` на `8001` в одном месте и перезагрузите nginx).
...
3. **Миграции**  
   `cd /opt/itflux/Generator && python manage.py migrate`
...
В `02_lk_generator/ITFLUX_INTEGRATION.md` (или прежнем `GENUROK_INTEGRATION.md`) — формат JWT и контракт (совпадает с `LessonTokenView` в `Cabinet/views.py`).

## Прокси домашки: `403 Authentication credentials were not provided`

Платформа «Цифровой поток» дергает ЛК **с сервера** (`GET /api/homework/assignment/<id>/` или опционально отдельный POST), с заголовками:
...
Чаще всего **поддомен `lk` смотрит на тот же nginx `server`, что и генератор** (или в `.env` при сборке указан `VITE_LK_URL=http://itflux...` без `lk.`).
...
- В **nginx** для `lk.itflux.ru` должен быть **отдельный** `server { ... }` с `proxy_pass` на **бэкенд кабинета**, а не на тот же upstream, что `tes.itflux.ru`.
