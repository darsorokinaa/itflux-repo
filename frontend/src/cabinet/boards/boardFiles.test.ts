...
import { externalizeSceneFiles } from "./boardFiles";

function bigPngDataUrl(bytesLength: number): string {
  // Невалидный PNG по содержимому, но dataURL для теста размера; upload мокаем
...
describe("externalizeSceneFiles", () => {
  it("оставляет маленькие dataURL без upload", async () => {
    const upload = vi.fn();
    ...
    expect(out.f1.dataURL).toBe(tiny);
  });
